function [spikes] = PostStimProcess(tau, N, alpha1, alpha2, lambda1, lambda2, k1, k2)
% POSTSTIMPROCESS generates a renewal process with parameters lambda1 and
% lambda2, k1 and k2 after a stimulus comes on which supresses
% the firing rate

% INPUTS
% tau = time constant which controls the refractory rate (in ms)
...
for cell = 1:N
    ...
    % Fill in bursts
    for i = 2:L
        r(i) = r(i-1)+(lambda1-r(i-1))*dt1/tau;
        if r(i)*dt1 > x(i) % if this condition holds...
